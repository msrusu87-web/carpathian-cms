<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    <div x-data="{
        ...
        async generate() {
            ...
            if (!hasBasicData) {
                window.Filament.notifications.notify({ type: 'warning', message: 'Please enter at least a title/name before generating content' });
                return;
            }
            ...
            try {
                ...
                if (data.success) {
                    ...
                    if (typeof window.Filament !== 'undefined' && window.Filament.notifications) {
                        window.Filament.notifications.notify({ type: 'success', message: message });
                    }
                } else {
                    console.error('❌ Generation failed:', data);
                    if (typeof window.Filament !== 'undefined' && window.Filament.notifications) {
                        window.Filament.notifications.notify({ type: 'danger', message: data.error || 'Generation failed' });
                    } else {
                ...
                if (typeof window.Filament !== 'undefined' && window.Filament.notifications) {
                    window.Filament.notifications.notify({ type: 'danger', message: 'Failed to generate content: ' + error.message });
                } else {
        ...
        <div class="bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-lg border-2 border-purple-200 dark:border-purple-800 p-6">
            ...
            <button
                ...
                @click="generate()"
                :disabled="generating"
                style="background: linear-gradient(to right, #9333ea, #db2777); color: white !important; font-weight: bold; padding: 12px 24px; border-radius: 8px; cursor: pointer; width: 100%; display: flex; align-items: center; justify-content: center; gap: 8px; transition: all 0.2s;"
                class="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 disabled:from-gray-400 disabled:to-gray-500 text-white font-bold py-3 px-6 rounded-lg transition-all duration-200 flex items-center justify-center gap-2"
                    style="background: linear-gradient(to right, rgb(147, 51, 234), rgb(219, 39, 119)); color: white !important; font-weight: bold; padding: 12px 24px; border-radius: 8px; cursor: pointer;"
                >
