        ...
        <div class="grid lg:grid-cols-2 gap-12 mb-16" x-data="{ selectedImage: 0, quantity: 1, addedToCart: false }">
                    ...
                    <div class="grid grid-cols-4 gap-4">
                        @foreach($images as $index => $image)
                            <div @click="selectedImage = {{ $index }}" 
                                ...
                                <img src="{{ asset($image) }}" alt="Thumbnail {{ $index + 1 }}" class="w-full h-24 object-cover">
                            </div>
                        @endforeach
                    </div>
                @endif
            ...
            <div class="animate-slide-in-right">
                ...
                <div class="bg-gradient-to-r from-blue-50 to-purple-50 rounded-2xl p-8 mb-8 border-2 border-blue-200">
                    @if($product->sale_price)
                        <div class="flex items-center gap-6 mb-2">
                            <span class="text-3xl text-gray-400 line-through font-semibold">{{ number_format($product->price, 0, ',', '.') }} RON</span>
                            <span class="bg-red-500 text-white text-lg px-4 py-2 rounded-full font-bold">
                                SAVE {{ number_format($product->price - $product->sale_price, 0, ',', '.') }} RON
                            </span>
                        </div>
                        <div class="text-6xl font-black text-green-600">
                            {{ number_format($product->sale_price, 0, ',', '.') }} <span class="text-3xl">RON</span>
                        </div>
                    @else
                        <div class="text-6xl font-black bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                            {{ number_format($product->price, 0, ',', '.') }} <span class="text-3xl">RON</span>
                        </div>
                    @endif
                    <p class="text-sm text-gray-600 mt-2"><i class="fas fa-info-circle mr-1"></i>Prețuri includ TVA</p>
                </div>
                ...
                <div class="flex gap-4 mb-6">
                    ...
                    <button @click="addedToCart = true; setTimeout(() => addedToCart = false, 3000)" 
                            class="flex-1 bg-gradient-to-r from-green-500 to-green-600 text-white px-8 py-4 rounded-xl hover:from-green-600 hover:to-green-700 text-xl font-bold shadow-xl hover:shadow-2xl transform hover:scale-105 transition-all duration-300">
                        <i class="fas fa-shopping-cart mr-2"></i><span x-text="addedToCart ? 'Adăugat în Coș! ✓' : '{{ __('messages.add_to_cart') }}'"></span>
                    </button>
                </div>

                <!-- Buy Now & Contact -->
                <div class="flex gap-4">
                    <a href="{{ route('contact') }}" class="flex-1 text-center bg-gradient-to-r from-blue-500 to-blue-600 text-white px-8 py-4 rounded-xl hover:from-blue-600 hover:to-blue-700 text-lg font-bold shadow-lg hover:shadow-xl transition">
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden mb-16 animate-fade-in-up" x-data="{ activeTab: 'description' }">
            ...
            <div class="p-12">
                ...
                <div x-show="activeTab === 'description'" class="prose prose-lg max-w-none">
                    <h2 class="text-3xl font-bold mb-6 text-gray-900">Descriere Detaliată</h2>
                    {!! $product->getTranslation('content', app()->getLocale()) ?? '<p class="text-xl leading-relaxed text-gray-700">Portal Corporate reprezintă soluția completă pentru companii care doresc o prezență online profesională și scalabilă. Această platformă oferă toate funcționalitățile necesare pentru gestionarea eficientă a conținutului corporativ, integrarea echipelor și comunicarea cu clienții.</p>
                    
                    <h3 class="text-2xl font-bold mt-8 mb-4">De ce să alegi Portal Corporate?</h3>
                    <ul class="space-y-3 text-lg">
                        <li><i class="fas fa-check-circle text-green-600 mr-2"></i>Platformă robustă și scalabilă pentru orice dimensiune de business</li>
                        <li><i class="fas fa-check-circle text-green-600 mr-2"></i>Interfață intuitivă pentru managementul conținutului</li>
                        <li><i class="fas fa-check-circle text-green-600 mr-2"></i>Module integrate: blog, portofoliu, echipă, servicii</li>
                        <li><i class="fas fa-check-circle text-green-600 mr-2"></i>Sistem avansat de permisiuni pentru utilizatori multipli</li>
                        <li><i class="fas fa-check-circle text-green-600 mr-2"></i>Optimizare SEO completă și integrare analytics</li>
                        <li><i class="fas fa-check-circle text-green-600 mr-2"></i>Design responsive și performant pe toate dispozitivele</li>
                    </ul>
                    
                    <h3 class="text-2xl font-bold mt-8 mb-4">Inclus în pachet:</h3>
                    <div class="grid md:grid-cols-2 gap-6">
                        <div class="bg-blue-50 p-6 rounded-xl">
                            <h4 class="font-bold text-xl mb-3 text-blue-900"><i class="fas fa-code mr-2"></i>Development</h4>
                            <ul class="space-y-2 text-gray-700">
                                <li>• Arhitectură personalizată</li>
                                <li>• Cod optimizat și documentat</li>
                                <li>• Integrări API custom</li>
                                <li>• Testing complet QA</li>
                            </ul>
                        </div>
                        <div class="bg-purple-50 p-6 rounded-xl">
                            <h4 class="font-bold text-xl mb-3 text-purple-900"><i class="fas fa-paint-brush mr-2"></i>Design</h4>
                            <ul class="space-y-2 text-gray-700">
                                <li>• UI/UX profesional</li>
                                <li>• Branding personalizat</li>
                                <li>• Grafică originală</li>
                                <li>• Animații moderne</li>
                            </ul>
                        </div>
                    </div>' !!}
                </div>
                ...
                <div x-show="activeTab === 'specifications'">
                    <h2 class="text-3xl font-bold mb-8 text-gray-900">Specificații Tehnice</h2>
                    <div class="grid md:grid-cols-2 gap-8">
                        <div>
                            <h3 class="text-xl font-bold mb-4 text-blue-600">Tehnologii</h3>
                            <table class="w-full">
                                <tbody class="divide-y">
                                    <tr class="hover:bg-gray-50">
                                        <td class="py-4 font-semibold text-gray-700">Framework</td>
                                        <td class="py-4 text-gray-900">Laravel 11</td>
                                    </tr>
                                    <tr class="hover:bg-gray-50">
                                        <td class="py-4 font-semibold text-gray-700">Frontend</td>
                                        <td class="py-4 text-gray-900">TailwindCSS + Alpine.js</td>
                                    </tr>
                                    <tr class="hover:bg-gray-50">
                                        <td class="py-4 font-semibold text-gray-700">Database</td>
                                        <td class="py-4 text-gray-900">MySQL / PostgreSQL</td>
                                    </tr>
                                    <tr class="hover:bg-gray-50">
                                        <td class="py-4 font-semibold text-gray-700">Cache</td>
                                        <td class="py-4 text-gray-900">Redis</td>
                                    </tr>
                                    <tr class="hover:bg-gray-50">
                                        <td class="py-4 font-semibold text-gray-700">Server</td>
                                        <td class="py-4 text-gray-900">Nginx / Apache</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        <div>
                            <h3 class="text-xl font-bold mb-4 text-purple-600">Performanță</h3>
                            <table class="w-full">
                                <tbody class="divide-y">
                                    <tr class="hover:bg-gray-50">
                                        <td class="py-4 font-semibold text-gray-700">Load Time</td>
                                        <td class="py-4"><span class="bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm font-bold">&lt; 1s</span></td>
                                    </tr>
                                    <tr class="hover:bg-gray-50">
                                        <td class="py-4 font-semibold text-gray-700">PageSpeed Score</td>
                                        <td class="py-4"><span class="bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm font-bold">95+</span></td>
                                    </tr>
                                    <tr class="hover:bg-gray-50">
                                        <td class="py-4 font-semibold text-gray-700">Concurrent Users</td>
                                        <td class="py-4 text-gray-900">10,000+</td>
                                    </tr>
                                    <tr class="hover:bg-gray-50">
                                        <td class="py-4 font-semibold text-gray-700">Uptime</td>
                                        <td class="py-4"><span class="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm font-bold">99.9%</span></td>
                                    </tr>
                                    <tr class="hover:bg-gray-50">
                                        <td class="py-4 font-semibold text-gray-700">Security</td>
                                        <td class="py-4"><span class="bg-purple-100 text-purple-800 px-3 py-1 rounded-full text-sm font-bold">SSL + WAF</span></td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                    ...
                    <div class="mt-8 bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl p-8">
                        <h3 class="text-xl font-bold mb-4">SKU: {{ $product->sku }}</h3>
                        <div class="grid md:grid-cols-3 gap-6">
                            <div>
                                <p class="text-gray-600 mb-1">Timp Implementare</p>
                                <p class="text-2xl font-bold text-blue-600">4-6 săptămâni</p>
                            </div>
                            <div>
                                <p class="text-gray-600 mb-1">Suport Tehnic</p>
                                <p class="text-2xl font-bold text-green-600">24 luni inclus</p>
                            </div>
                            <div>
                                <p class="text-gray-600 mb-1">Training</p>
                                <p class="text-2xl font-bold text-purple-600">4 sesiuni</p>
                            </div>
                        </div>
                    </div>
                </div>
