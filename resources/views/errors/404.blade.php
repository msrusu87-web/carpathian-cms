...
<head>
    ...
    <title>{{ __('messages.error_404') ?? 'Page not found' }}</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="min-h-screen bg-gray-50 flex items-center justify-center p-6">
    <div class="max-w-xl w-full bg-white shadow rounded-xl p-8">
        <h1 class="text-2xl font-bold text-gray-900">{{ __('messages.error_404') ?? 'Page not found' }}</h1>
        <p class="mt-2 text-gray-600">
            {{ __('messages.page_not_found') ?? 'The page you are looking for does not exist or was moved.' }}
        </p>
        ...
        <div class="mt-8 border-t pt-4 text-sm text-gray-500">
            <div><span class="font-semibold">HTTP</span>: 404</div>
